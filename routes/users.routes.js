...
router.route('/loguser').post(function (req, res) {
    User_model.findOne({ id: req.body.id }, function (err, doc) { return __awaiter(_this, void 0, void 0, function () {
        var newUser;
                    ...
                    if (doc) {
                        User.find({ id: req.body.id }, function (err, currentUser) {
                            if (currentUser.avatar != req.body.avatar) {
                                User.findOneAndUpdate({ id: req.body.id }, { $push: { avatar: req.body.avatar }, function: function (err, user) {
                                        if (!err) {
...
router.route('/lastfiveusers').get(function (req, res) {
